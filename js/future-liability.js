/*
 * ===== このツールの確定ルール(2026-08-28,29 ユーザー決定。変更時は必ずここと照合し、
 *        抵触する修正は勝手に行わず選択肢を提示して確認を取ること) =====
 * 1. BSは常に左右バランスし、バーの頭(上端)は左右でそろう(債務超過時も)
 * 2. 債務超過バンドは金額に関わらず固定の縦幅(NEG_BAND_H)。色は#9c3d4c+白抜き2行。
 *    実寸がバンドを超える場合はバンドを伸ばさず、縦スケール(pxPerYen)を圧縮して吸収する
 * 3. 簿外資産ゾーンはバランス不要。備えが将来負債を超えたら、その場で縦に押し上げて表示
 * 4. セグメントのラベルは帯内必須表示。「名前+金額」が横幅・縦幅のどちらかに
 *    収まらないときは数字のみ(名前はツールチップ)。隣とラベルが重なる場合は重なる側を非表示
 * 5. 一時払対策は入力額を流動資産→その他資産→固定資産の順で振り替え、
 *    「将来負債対策分」の点線枠(全額分)を同じ位置に残す(オンバランス層のバランス維持)
 * 6. 保険差益は ①対策分の枠を上端から埋める(ブルーグリーン#45939b白抜き・緑点線枠)
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ===== 簿外資産(将来負債の備え): 生命保険金+一時払対策+保険差益で足りない分が「不足分」。
  // 合計欄には 充当額+不足分(=最低でも将来負債合計) を表示する =====
  const OFF_BALANCE_ASSET_IDS = ['lifeInsurance', 'otherCoverage', 'insuranceGain'];
  ...
  function updateChart(dataByBar, pxPerYen, showValues, labelOnly) {
    ...
    initChartOnce();
    // 債務超過(マイナス値)のはみ出しバンドの最大高さ(px)。見出し等をその分だけ下げる
    const maxNegPx = Math.max(0, ...Object.values(dataByBar).map((segs) =>
      segs.reduce((s, x) => s + (x.value < 0 ? Math.abs(x.value) * pxPerYen : 0), 0)));
    updateLayout(maxNegPx);
    Object.entries(dataByBar).forEach(([barKey, segments]) => {
      // 債務超過のバーは、正の要素を超過分だけ基準線の下から積み始める。
      // こうすると正味合計が同じ左右のバーの頭がそろう(BSはバランスしているように見える)
